...
- Thread 
  ...
  CUDA的每一个线程都是工作在CUDA core上的，CUDA 线程与 CPU 线程不同，CUDA 线程非常轻量级，可提供快速的上下文切换。其原因在于 GPU  中的寄存器容量大，并且有基于硬件的调度程序。在GPU线程上下文存在于寄存器中，而 CPU 的线程上下文存在于较低层次的存储结构中，  如高速缓存（L1，L2 ,L3 cache...）。因此，在GPU中当一个线程处于空闲/等待状态时，另一个准备就绪的 线程就可以开始执行，几乎没有延迟。每个 CUDA 线程必须执行相同的内核（相同的指令），并独立处理不同的数据（SIMT）。 
...
每个SM有一级缓存，但是二级缓存由所有SM共享，不属于某个SM所有。
...
> 问：一个SM上，在某一个时刻，只会有一个线程束被执行吗？
>
> **现代 GPU 的 SM 支持多线程束并发执行**。例如：
>
> - **Fermi 架构**：每个 SM 包含 2 个 Warp Scheduler（线程束调度器），可同时发射指令到 2 个不同的线程束。
> - **Pascal 及之后架构**：SM 被划分为多个子块（如 4 个子块），每个子块可独立调度一个线程束，实现多个线程束并行执行。
>
> **SP（流处理器）数量与线程束关系**：每个线程束包含 32 个线程，但 SP 数量通常远大于 32（如 128 个 SP 的 SM），通过多线程束调度充分利用计算资源
>
> 参考：https://www.zhihu.com/question/445590537/answer/10468941897
...
在 Fermi 架构之前，GPU 一次只能执行一个内核。Fermi上支持同时并发执行内核(启动并执行多个核函数)。并发执行内核允许执行一些小的内核程序来充分利用GPU，如下图：
...
这是一个平衡问题，当kernel占用的资源较少，那么更多的线程处于活跃状态，相反则线程越少。
...
总结一下就是：**寄存器资源影响SM里的线程束(warp)数量，共享内存资源影响SM里的线程块(block)数量**

上面讲的主要是线程束，如果从逻辑上来看线程块的话，可用资源的分配也会影响[常驻线程块](# 常驻线程块)的数量。
...
**SP (Streaming Processor)** 和 **CUDA Core** 是完全相同的概念：它们都是指GPU中能执行单精度浮点运算的最基本计算单元，NVIDIA在早期文档中使用"SP"这一术语，后来的营销和技术文档中更多地使用"CUDA Core"这一术语。
...
一级缓存，二级缓存，共享内存，以及只读和常量缓存，他们的关系如下图：
 ...
 <img src="assets/readme/5-1.png" alt="5-1"  />
...
常量内存和只读缓存区别：**常量缓存对于统一读取（读同一个地址）执行更好，只读缓存适合分散读取**。

下面我们了解下共享内存的生命周期和读取性质。

共享内存是在他所属的线程块被执行时建立，线程块执行完毕后共享内存释放，线程块和他的共享内存有相同的生命周期。对于每个线程对共享内存的访问请求
...
后面的全章内容都是基本围绕如何避免访问冲突，高效的使用共享内存来展开的。
...
最优访问模式（并行不冲突）：
![5-2](assets/readme/5-2.png)

不规则的访问模式（并行不冲突）：

![5-3](assets/readme/5-3.png)

不规则的访问模式（并行可能冲突，也可能不冲突）

![5-4](assets/readme/5-4.png)
这时候又两种可能
...
![5-5](assets/readme/5-5.png)
**同一个线程束中的两个线程访问同一个地址不会发生冲突，一个线程读取后广播告诉有相同需求的线程。但是对于写入，这个就不确定了**，结果不可预料。
上面我们介绍的存储体宽度是4的情况，宽度是8的情况同样，但是宽度变宽了，其带宽就变宽了，比如，我们之前一次只能取4 byte，现在可以取8 byte，这时候如果有两个线程访问同一个存储体，按照我们前面的解释，一种是访问同一个地址，这时候通过广播来解决冲突，还有一种冲突是需要用等待解决的，当存储体变宽了，如果一个线程想要存储体里左边的数据，而一个线程想要右边的数据，这时候是不冲突的，因为存储体是够宽的。

宽度为8字节的图如下：

![5-6](assets/readme/5-6.png)

下图显示64位宽的存储体无冲突访问的一种情况，每个bank被划分成了两部分

![5-7](assets/readme/5-7.png)

下图是另一种无冲突方式：

![5-8](assets/readme/5-8.png)

一种冲突方式，两个线程访问同一个小桶：

![5-9](assets/readme/5-9.png)

另一种冲突方式，三个线程访问同一个小桶

![5-10](assets/readme/5-10.png)

### 内存填充

存储体冲突会严重影响共享内存的效率，那么当我们遇到严重冲突的情况下，可以使用填充的办法让数据错位，来降低冲突。
假如我们当前存储体内的数据罗列如下，这里假设共5个存储体，实际是32个

![5-11](assets/readme/5-11.png)
...
![5-11-1](assets/readme/5-11-1.png)
然后编译器会将这个二维数组重新分配到存储体，因为存储体一共就5个，我们每一行有6个元素，所以有一个元素进入存储体的下一行，这样，所有元素都错开了，就不会出现冲突了。

![5-12](assets/readme/5-12.png)
...
![image-20251204103447291](assets/readme/image-20251204103447291.png)
...
## 写完核函数后要从如下几点思考还有没有优化空间

写完了native版本后，需要
1.看一下是否有分支，避免线程束分化
2.是否可以把全局内存拷贝到共享内存/计算器上，在共享内存/计算器上做计算，共享内存要避免 bank 冲突
3.预取内存
...
CUDA Streams：CUDA流是CUDA编程模型中用于管理和调度并行操作的抽象概念。使用CUDA流，可以将多个GPU操作组织成不同的流，并发地执行这些流中的操作。CUDA流提供了一种灵活的方式来管理GPU资源并优化并行计算。

CUDA Graphs：CUDA图是一种高级的CUDA编程技术，用于提前定义和优化GPU操作的执行顺序。通过创建和使用CUDA图，可以减少GPU操作之间的依赖关系，从而提高并行性和性能。CUDA图允许开发者以图的形式表示和管理GPU操作，这使得在某些情况下可以实现更高级的优化和调度。

## CUDA 流和 CUDA 图

CUDA Streams：是 CUDA 编程模型中用于管理和调度并行操作的抽象概念。流允许将 GPU 操作组织到不同的队列中，实现操作的并发执行。同一流内的操作按顺序执行，而不同流之间的操作可以并发执行，默认流与所有其他流同步。CUDA 流主要用于实现计算与数据传输重叠、多 GPU 任务并行处理和任务级并行性优化，为开发者提供了灵活管理 GPU 资源的方式。

CUDA Graphs：是一种高级 CUDA 编程技术，用于提前定义和优化 GPU 操作的执行顺序。图以有向无环图形式表示操作及其依赖关系，支持"一次定义，多次执行"的模式。**CUDA 图通过减少 CPU-GPU 交互开销、支持内核融合和其他图级优化，显著降低启动延迟并提高 GPU 利用率。**相比 CUDA 流的动态灵活性，CUDA 图更适合于重复执行相同操作序列的固定工作流场景，可视为 CUDA 流的进阶技术，为追求极致性能的应用提供了更优的解决方案。
