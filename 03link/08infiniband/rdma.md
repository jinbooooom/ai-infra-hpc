...
3）**DPDK在低负荷场景下会造成CPU的无谓空转，RDMA不存在此问题**（DPDK没有中断，只有轮询）
...
dpdk 能够绕过内核协议栈，本质上是得益于 UIO 技术，通过 UIO 能够拦截中断，并**重设中断回调行为，从而绕过内核协议栈后续的处理流程**。
...
QPC全称是Queue Pair Context，用于存储QP相关属性。驱动程序里面是有储存QP的软件属性的，既然我们可以在软件里储存QP的属性，为什么还要用使用QPC呢？因为**QPC主要是给硬件看的，也会用来在软硬件之间同步QP的信息**。
...
因为**不可靠的服务类型没有重传和确认机制**，所以产生CQE表示硬件**已经将对应WQE指定的数据发送出去了**。以前说过UD只支持SEND-RECV操作，不支持RDMA操作。所以对于UD服务的两端，CQE产生时机如下图所示。
...
当我们创建了QP之后，系统是需要保存状态数据的，比如QP的metadata，拥塞控制状态等等，除去QP中的WQE、MTT、MPT，一个QP大约对应375B的状态数据。这在以前RNIC的SRAM比较小的时候会是一个比较重的存储负担，所以以前的RDMA工作会有QP Sharing的研究，就是不同的处理线程去共用QP来减少meta  data的存储压力，但是这会带来一定的性能的损失。现在新的RNIC的SRAM已经比较大了，Mellanox的CX4、CX5系列的网卡的SRAM大约2MB（2MB除以375B，可以开辟很多的QP了），所以现在新网卡上，大家还是比较少去关注QP带来的存储开销，除非你要创建几千个，几万个QP。

其次，RNIC是包含多个Processing Unit(PU)的，同时由于QP内的请求处理是具有顺序的，且为了`避免cross-PU的同步`（多个PU处理同一个QP，就需要跨PU同步来保证请求的顺序性），一般而言我们认为一个QP对应一个PU来处理。**所以，可以在一个线程内建立多个QP来加速数据处理，避免RDMA程序性能瓶颈卡在PU的处理上**。当QP数量超过PU数量时，边际性能提升会减少。
...
![img](assets/rdma/1143917-20180314125130422-1228589611.png)

在无损网络里，交换机 1 的端口 G 出口被 A～F 打满了，G 发生拥塞，于是对连到交换机 2 的这条链路发出流控（PAUSE/PFC），让交换机 2 减速。问题在于：这条被“踩刹车”的链路上，不只有 F→G 的流量，X→Y 的流量也走同一物理链路/优先级队列，所以交换机 2 只能整体把这条链路上该优先级的发送速率压到 20%，导致与 G 毫不相干的 X→Y 也从潜在的 80% 被拖到 20%，这就是拥塞传播。

拥塞控制要做的，就是尽量不要通过“粗粒度的端口/优先级流控”来解决问题，而是识别真正导致 G 拥塞的那些流（A～F），只降低它们的注入速率，让它们的总速率匹配 G 的处理能力；这样 A～F 的体验不会明显变差（毕竟，它们的数据包无论如何也要等待），但像 X→Y 这类不经过拥塞点的流就不会被误伤，仍然可以充分利用剩余带宽。当前 RoCE 的方案就是用 ECN 这类显式拥塞通知，把“是谁把队列挤满了”精确反馈给源端，再对对应 QP 做端到端限速。

### PFC（Priority-based Flow Control）

PFC是以太网的“按优先级的 PAUSE”机制，用于构建无损以太网，尤其是承载 RoCEv2 流量时：传统 802.3x PAUSE 会直接把整个端口都停掉，而 PFC 在数据帧里携带优先级信息（Priority/TC），交换机可以只对某一个或几个优先级发送 PAUSE 帧，从而“只暂停这些优先级上的流量”。**RoCEv2 一般跑在被标成“无损优先级”的队列上，当该优先级的队列快被填满时，交换机向对端端口发送 PFC 帧，请求对端在这个优先级上暂时停止或减慢发送，让队列有时间排空，以避免丢包**；这样可以在保持 RoCEv2 无损的同时，不至于把其他 Best-Effort 流量一起停掉。**缺点是：PFC 仍然是“端口/优先级粒度”的粗流控，很容易造成拥塞传播**，所以 RoCEv2 还要叠加 ECN + DC-QCN，从端到端角度精准地对造成拥塞的 QP/流量降速，减少对无辜流的影响。

### ECN（Explicit Congestion Notification）

显式拥塞通知ECN最初在RFC  3168文档为TCP/IP协议所定义，它通过在IP头部嵌入一个拥塞指示器和在TCP头部嵌入一个拥塞确认实现。兼容ECN的交换机和路由器会在检测到拥塞时对网络数据包打标记。IP头部的拥塞指示也可以用于RoCEv2的拥塞控制。下面是IP头部的前四个帧的格式：

![img](assets/rdma/1143917-20180314145004049-751939265.png)

下面是整个IP头部的帧格式：

![img](assets/rdma/1143917-20180314145744455-901717269.png)

ECN 的思路是：先不丢，而是在 IP / TCP 头里打一个标记，显式通知拥塞。

IP 头里有 2 bit ECN 域（RFC 3168 定义）：

- `00`：不支持 ECN；
- `10` / `01`：ECT(0)/ECT(1)，表示“这条流支持 ECN”（ECN-Capable Transport）；
- `11`：CE (Congestion Experienced)，表示“这个包在路上经历过拥塞队列”。

路由器/交换机的行为：

- 正常情况下：只转发，不改 ECN；某个队列长度超过阈值时：
  - 如果收到的包 ECN 域是 `10` 或 `01`（说明对端支持 ECN），就把它改成 `11`（CE），表示“你刚经过一个拥塞点”；
  - 这样就 不用立刻丢包，还能把拥塞信息带到终点。

终点和发送方的行为：

- 接收端看到 IP 头 ECN=CE（再结合 TCP 头里的相关位），知道“路上出现过拥塞”，就通过协商好的机制告诉发送方；
- 发送方接到这个“拥塞信号”后，主动降低发送速率（类似丢包时的减速），以缓解拥塞。

### QCN（Quantized Congestion Notification）

IEEE标准(802.Qau)中，量化拥塞通知以有限带宽时延的以太网网络为长期保活的网络流提供了拥塞控制。这是IEEE数据中心桥接(DCB)协议套件中的一部分，还包括ETS,PFC,和DCBX。QCN在二层以太网络上制定，并且是针对硬件实现的。QCN适用所有的以太网帧和所有的传输，并且主机端和交换机端的行为也在标准中详细规定。QCN的配置和信息提取可以适用mlnx_qcn命令

### CNP（Congestion Notification Packet）

拥塞通知报文。

### Infiniband 和 RoCEv2 的拥塞控制简要概述

Infiniband 和 RoCEv2 的拥塞控制都希望在无损前提下避免“拥塞传播”，但机制不同：RoCEv2 依赖以太网上的 ECN 标记和 DC-QCN 算法，IB 则在链路层使用基于 credit 的流控【`Credit-Based Flow Control（基于信用的流量控制）`】，加上可选的 IB Congestion Control（通过在 IB 头里打 FECN/BECN 标记，并由接收端或交换机生成 CNP，通知源端对特定 QP 降速，再按配置的算法平滑恢复速率）。IB 的思想和 DC-QCN 类似，都是“谁造成拥塞就对谁限速”，但具体报文字段、参数和控制律是 IB 规范独立定义的，而不是直接复用 DC-QCN。

从协议栈角度看，RoCEv2 跑在 UDP/IP/以太网上，它本身不具备无损特性，需要依赖以太网侧的 PFC/ECN 等机制来构建“近似无损”的行为；因此 DC-QCN 采用 IP 头里的 ECN 位来感知交换机队列的拥塞程度，并用 CNP + 源端速率控制算法在端到端层面限制 RoCEv2 QP 的注入速率，用软件/固件上的拥塞控制补上“以太网天生有丢包”的短板。Infiniband 则在链路层就定义了 credit-based 的无损流控：每个端口知道对端还有多少 buffer credit，只在有 credit 时发包，先从物理层面避免丢包；在这个基础上，IB CC 再用 FECN/BECN 标记和 CNP 做“细粒度的端到端限速”，从简单的“整条链路一起 PAUSE”进化到“只对经过拥塞路径的 QP 降速”，尽量减少对无辜流量的影响。

更细一点对比如下：

1）拥塞检测位置：RoCEv2 使用交换机在出口队列上根据队列长度阈值给 IP 头打 ECN（CE）标记；IB 在本地端口/虚通道（VL）队列上根据队列深度检测拥塞，在 IB BTH 或专门扩展头部里打 FECN/BECN。

2）拥塞信号返回：RoCEv2 的接收端 HCA 看到 ECN 标记后合并成 CNP，通过 IP/UDP 路由回源端；IB 的 CNP 是 IB 层报文，可以由接收端或中间交换机直接生成，沿 IB 拓扑返回到源端。

3）限速对象：RoCEv2/DC-QCN 按 QP/流来维护发送速率和 α 参数；IB CC 同样按 QP 或 VL 粒度控制速率，只是算法形式和参数由 IB 规范和厂商实现决定。

4）与底层流控关系：RoCEv2 必须配合 PFC，否则 ECN 通知之前就已经丢包；IB 则先有 credit 流控保证不丢，再用 IB CC 做拥塞热点的“责任分摊”。

5）分层归属：credit-based 无损流控属于链路层，是 per-link、per-VL 的物理层/链路层行为；FECN/BECN 标记和基于 CNP 的拥塞控制决策发生在 IB 的网络层（路由和队列管理这一层），在转发报文时根据队列状态修改头部并触发 CNP，从而在端到端视角上对特定 QP 实施限速。

可以简单记为：RoCEv2 是“在有丢包倾向的以太网上用 ECN/DC-QCN 搞出一层端到端拥塞控制”，IB 是“在本身无损的 IB 链路上再加一层更聪明的拥塞控制，让热点对自己负责”。

### DC-QCN算法

**通俗的表达**：

**DC‑QCN 里的 QCN，可以把它想成“交换机给每条流打分、网卡按拥塞程度决定发送的多快”的机制。**交换机会一直观察自己端口上的队列长度：队列很短就什么都不做，队列开始变长、接近拥塞时，就在经过的 RoCEv2 数据包上用 ECN 打一个“这里有点堵”的标记。这些带标记的包到了接收端 NIC，接收端不会每包都大惊小怪，而是在一个很短的时间窗口里统计：某个 QP 最近到底收到了多少个“这里在堵”的提示，然后合并成一条专门的 CNP 通知包，回发给源端 NIC，相当于告诉它：“你这条流最近让交换机的队列很紧张了，分数变高了”。源端 NIC 收到 CNP 后，会给这个 QP 维护一个“拥塞度”参数 α，α 越大说明越拥挤，然后按照 DC‑QCN 规定的公式把发送速率踩下去一点（比如 `new_rate = old_rate * (1 - α/2)`），而在一段时间没有再收到 CNP 时，又按“快速恢复 → 稍微加速 → 更积极加速”几个阶段慢慢把速率提回去。这样一来，网络不用靠“丢包”或者“一刀切 PAUSE 整个端口”来表达拥塞，而是通过 QCN 提供的这种“量化的拥塞打分 + CNP 通知 + α 控制”，做到精确地让那些真正制造拥塞的 RoCEv2 流自己减速，同时尽量不给无辜流量添堵，又能保持整体带宽利用率。

更细一点对应关系是：

- 交换机端：不直接算分，只是根据队列长度给包打 ECN 标记【ECN 标记】。
- 接收端 NIC：在短时间内统计这些 ECN 标记，合并成 CNP【CNP 通知】，这相当于“告诉源端：这条流最近被标记得有点多了”。
- 源端 NIC：真正维护一个随时间变化的“分数”【拥塞度参数 α】，用 CNP 到达的有无/频率来更新 α（CNP 多 → α 变大，CNP 少 → α 变小），然后根据 α 决定 `new_rate = old_rate * (1 - α/2)` 之类的减速力度
...
![img](assets/rdma/1143917-20180314202018710-1495055209.png)
...
![img](assets/rdma/1143917-20180314202402711-1723592878.png)

带有ECN标记的数据包被传播到接收方的网卡上。接收方网卡创建一个CNP并把它发送给ECN所标记的数据包的发送方。CNP数据包包括被标记的QP的信息。当CNP被发送方网卡收到时，它会基于下面描述的算法来降低指定QP的传输速率。

![img](assets/rdma/1143917-20180314153615825-355438496.png)

![img](assets/rdma/1143917-20180314202754190-381665703.png)

DC-QCN降速算法通过下面的图表来描述。简而言之，如果QP基于内部定时器和发送字节计数器，算法会持续的增加发送速率，一旦收到CNP包，就会对指定QP进行降速。除此之外，它还维持一个叫做α的参数，它反映了网络中的拥塞度，用于降速计算。

![img](assets/rdma/v2-fc3d44d23abad467e63dfa55ae7aaf94_1440w.jpg)
...
target_rate 指【目标速率】，**即曾经跑到的、刚好没有出现拥塞时的那个速度**，在慢慢恢复的时候，把它当作一个想回去的目标值。
...
![img](assets/rdma/1143917-20180314210728316-799454570.png)



让AI总结【快速恢复，积极增加，超积极增加】

这三个阶段都是 “提速阶段”里的不同档位，区别只是“加速得有多猛”，目的都是：既能尽快恢复带宽，又不要一下子把队列重新挤爆。

**快速恢复【Fast Recovery】**

- 场景：刚刚降过速，当前速率比“拥塞前”低不少。
- 行为：每次触发提速事件时，把当前速率向 目标速率【target_rate】 靠近一半：
  (\text{current_rate} = (\text{current_rate} + \text{target_rate}) / 2)
- 直觉：一开始差距大，就可以一下子提很多；越接近 target_rate，加得越小，更谨慎。

**积极增加（保持探测）【Active Increase】**

- 当在快速恢复阶段已经提得差不多（提速事件次数超过某个门限），就进入积极增加。
- 行为：每次提速事件 加一个固定的小步长，线性往上爬。
- 直觉：已经大致恢复到“拥塞出现时的速率附近”，再往上就要慢慢探，看网络还能不能再多给一点带宽，这就是“保持探测”。

**超积极增加（保持探测）【Hyper‑Active Increase】**

- 再往后，如果持续一段时间都没触发降速事件，说明网络挺空的。
- 行为：每次提速事件 加更大的固定步长，比积极增加阶段更“猛”一点。
- 直觉：确认网络比较空闲，就可以更积极地“吃带宽”，提高吞吐，同样是边加边探测上限。

任意阶段一旦收到降速事件（CNP），就说明探过头了：

- 立刻把计数器清零，回到 快速恢复阶段，重新从一个相对保守的速率开始往回爬。

可以一句话总结成：

> 快速恢复：先用“对数方式”尽快回到之前的合理速率附近；
> 积极增加：确认差不多了后，慢慢线性加速，试探还能不能更快；
...
- [RDMA技术-Infiniband网络无损传输机制【13】](https://zhuanlan.zhihu.com/p/2002863033542783693)
