...
| 场景                           | GPU端使用                                    | CPU端使用                                          |
| :----------------------------- | :------------------------------------------- | :------------------------------------------------- |
| GPU线程间共享数据（同一block） | `__threadfence_block()` 或 `__syncthreads()` | -                                                  |
| GPU线程间共享数据（跨block）   | `__threadfence()`                            | -                                                  |
| CPU写入，GPU读取（统一内存）   | `__threadfence_system()`                     | `__sync_synchronize()`                             |
| GPU写入，CPU读取（统一内存）   | `__threadfence_system()`                     | `cudaDeviceSynchronize()` + `__sync_synchronize()` |
| CPU多线程同步                  | -                                            | `__sync_synchronize()`                             |
| 需要等待所有线程到达同步点     | `__syncthreads()`                            | -                                                  |
